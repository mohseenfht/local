...
if(isset($_POST["type"]) && $_POST["type"]=='add')
{
	...
	$course = $DB->get_records_sql("select * from {course} where id='".$cid."' ");
    
    /// For Course price start ///
    $coursePrice = $DB->get_records_sql("select * from {course_price} where course_id='".$cid."' ");
    ...
    /// For Course price end ///
	$product_code 	= filter_var($course[$cid]->id, FILTER_SANITIZE_STRING); //product code
	$product_qty 	= filter_var(1, FILTER_SANITIZE_NUMBER_INT); //product code
	
	$product_name 	= filter_var($course[$cid]->fullname, FILTER_SANITIZE_STRING); //product name
	$product_price 	= filter_var($cprice, FILTER_SANITIZE_NUMBER_FLOAT,FILTER_FLAG_ALLOW_FRACTION); //product price
	$product_image 	= filter_var(strip_tags($course[$cid]->summary), FILTER_SANITIZE_STRING); //product Image
	$product_category = filter_var($course[$cid]->category,FILTER_SANITIZE_STRING);
	$return_url  	= base64_decode($_POST["return_url"]); //return url
	    ...
	    $courses_my = $DB->get_records_sql("select * from {carts}  where user_id='".$USER->id. "' and code='".$product_code."' " );
            $counter = count($courses_my);
                 if( $counter == 0 ) {        
