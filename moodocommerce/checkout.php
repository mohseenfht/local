...
$output = '';
        ...
        $output .= html_writer::start_tag('div', array('class' => 'well well-sm' ,'style' => ' padding:44px;'));


              if($moodo->ccavenue_status == 1){
                 $output .= html_writer::start_tag('div', array('class' => 'span4 col-md-4 col-sm-6 col-xs-12' , 'style'=>'margin-left:0px' ));
                       $output .= html_writer::start_tag('input', array('class' => 'credit_card' , 'name' => 'payment_method' ,'value' => 'ccavenue' , 'type' => 'radio' )) . "<b>Credit Card (CCAvenue)</b>" ;
              ...
              //    $output .= html_writer::start_tag('div', array('class' => 'span4 col-md-4 col-sm-6 col-xs-12' , 'style'=>'margin-left:0px' ));
              //          $output .= html_writer::start_tag('input', array('class' => 'credit_card' , 'name' => 'payment_method' ,'value' => 'paypal' , 'type' => 'radio' )) . "<b>Credit Card (Paypal)</b>" ;
...
*/
                        	...
                        	if(!empty($products)){
                             ...
                             $table->head = array('Code','Course Name', 'Qty' , 'Unit Price','Action' );
                                        foreach ($products as $cart_itm) {
                                            ...
                                            $table->data[] = array($cart_itm->code, $cart_itm->name, $cart_itm->qty,$amounte,'<a href="'.$link.'"  class="btn btn-primary" title="Remove from the cart"  >x</a>');
    ...
    $output .= html_writer::start_tag('div' , array('class' => 'note note-primary'));
...
$output .=html_writer::start_tag('div' , array('class' => 'col-xs-12 col-md-12'));
        ...
        $output .=html_writer::start_tag('div' , array('class'=> 'span6 col-md-6' ));
        $output .=html_writer::start_tag('a' , array('class' => 'btn btn-primary', 'href' => $CFG->wwwroot."/course/"));
