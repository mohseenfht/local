...
if($_GET['method'] == 'ccavenue'){

              
                if(!empty($_POST['merchant_id']) && $_POST['merchant_id'] == $moodo->ccavenue_merchantid  ) {

                        include('../payment/ccavenue/Crypto.php');

                        $md5HashData = 'https://secure.ccavenue.com/transaction/transaction.do?command=initiateTransaction';
                        
                        $merchant_data='';
                        $working_key=$moodo->ccavenue_workingkey;   //Shared by CCAVENUES
                        $access_code=$moodo->ccavenue_access;    //Shared by CCAVENUES
                        ...
                        ////// Check on submit on every payment start  ///////////////// 
                                  ...
                                  $encode = base64_encode($url);
                        ////// Check on submit on every payment end  /////////////////
                         $vpcURL .= "&encRequest=".$encrypted_data; 
                         $vpcURL .= "&access_code=".$access_code; 
                         $md5HashData .= $vpcURL;
                         //header('location:'.$md5HashData);
                         echo "<script language=\"javascript\">top.location.href='$md5HashData'</script>";
                        exit;
