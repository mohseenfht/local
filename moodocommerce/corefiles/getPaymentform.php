...
if($_POST['paytype'] == 'ccavenue') {
    global $CFG,$USER,$DB;
    $i = 0;
    $goodprice = myCartTotol($USER->id);
    $creditt = getMyCredit($USER->id);
    $url = $USER->id."-ccavenue-".$creditt;
    $encode = base64_encode($url);


    $output  = '<form id="paypal_form" action="'.$CFG->wwwroot.'/local/moodocommerce/corefiles/process.php?method=ccavenue" method="POST" >';
    $output .= '<input type="hidden" name="merchant_id" value="'.$moodo->ccavenue_merchantid.'">';
    $output .= '<input type="hidden" value="'.date('Ymdhis').'" name="tid">
                <input type="hidden" value="utf-8" name="charset">
                <input name="currency" type="hidden" value="'.$moodo->currency_code.'" />';

    $output .= '<input name="billing_name" type="hidden" value="'.$USER->firstname.' '.$USER->lastname.' " /> 
                <input name="last_name" type="hidden" value="'. $USER->lastname.'" />
                <input name="billing_email" type="hidden" value="'.$USER->email.'" />
                <input type="hidden" value="'.$USER->city.' '.$USER->country.'" name="billing_address">
                <input type="hidden" value="billing_city" name="'.$USER->city.'">
                <input type="hidden" value="amount" name="'.$goodprice.'">
                <input type="hidden" value="language" name="EN">';
    $output .= '<input type="hidden" name="redirect_url" value="'.$CFG->wwwrootlocal."/moodocommerce/success.php?method=".$encode.'" />';
    $output .= '<input type="hidden" name="cancel_url" value="'.$CFG->wwwroot.'" />';


   
        
    $output .='<button  type="submit" class="btn btn-primary">Pay and Continue</button>';   
    $output .='</form>';
    echo $output;
}
